...
# This script does what the same script in master branch does,
# except I've put it in a function to facilitate easier use in the future.
# 
# Same notes apply re: leap_year function.


library(lubridate)

# in_file should be SWAT .std output file
# out_file should be a .csv
clean_swat_output <- function(in_file, out_file) {
  ...
  # Write output as CSV
  write.csv(x = df_all, file = out_file, quote = FALSE, row.names = FALSE)
  
}


clean_swat_output(in_file = "D:/SWAT/output.std", out_file = "D:/SWAT/output_clean.csv")
